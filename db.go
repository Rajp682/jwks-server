...
// exp is stored as a Unix timestamp integer.
...
// SeedKeys generates one valid key (expires in 1h) and one expired key (expired now)
// and saves both to the DB. Called on server start.
func (db *DB) SeedKeys() error {
	// Generate and save a key that expires in 1 hour.
	...
	// Generate and save a key that is already expired.
